...
**/
    ...
    function div_style_estados(numero_estado, puesto, estado) {
       ...
       if (numero_estado == 0) {
          ...
          text = text + "<div><a href=\"javascript:void%200\" onclick=\"comprueba_estado(" + numero_estado + "," + puesto + ",'" + estado + "'); return false;\"><img class=\"img_borde\" ";
          text = text + "src=\"images/blue.gif\" alt=\"boton_abierto\" ></img></a></div><div><small>Abierto</small></div>";
          }
       else if (numero_estado == 2) {
          text = text + "<div><a href=\"javascript:void%200\" onclick=\"comprueba_estado(" + numero_estado + "," + puesto + ",'" + estado + "'); return false;\"><img class=\"img_borde\" ";
          text = text + "src=\"images/rosso.gif\" alt=\"boton_comprobando\"></img></a></div><div><small>Comprobando</small></div>";
          }
       else if (numero_estado == 3) {
          text = text + "<div><a href=\"javascript:void%200\" onclick=\"comprueba_estado(" + numero_estado + "," + puesto + ",'" + estado + "'); return false;\"><img class=\"img_borde\" ";
          text = text + "src=\"images/berdea.gif\" alt=\"boton_listo\"></img></a></div><div>Listo</div>";
          }
	   else if (numero_estado == 4) {
          text = text + "<div><img src=\"images/sos.gif\" alt=\"boton_manual\"></img></div><div><small>SOS</small></div>";
       }
       text = text + "</div>";
       return text;
       }
    function comprueba_estado(numero_estado, puesto, estado) {
       if (estado != 0) {
          get_callpuesto(puesto);
          /*fichero callpuesto.js*/
          document.getElementById(estado).innerHTML = div_style_estados(2, puesto, estado);
          }
       else {
          alert("Fallo en estados.js: comprueba_estado");
          }
       }
